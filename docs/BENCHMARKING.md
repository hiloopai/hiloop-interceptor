...
`crates/hiloop-interceptor/benches/stdio_path.rs` and `benches/redaction.rs` (Criterion,
`harness = false`) record CPU and completion costs on the default capture path, run with
`cargo bench -p hiloop-interceptor`:
...
- `credential_redaction_mode/{serial,bounded_async}_{clean,credential}/{1k,64k,256k,1m,8m}` — the
  synchronous leakguard baseline and the production bounded blocking path over representative model
  JSON, including the default 8 MiB capture ceiling. Inputs are built outside the timed region and
  every credential is synthetic.
- `credential_redaction_concurrency/{64k,1m,8m}_credential/{1,2,4,16}` — aggregate throughput below,
  at, and beyond the two-slot large-body lane and four-slot small-body lane. This records real async
  scheduling and leakguard detector-parallel cost; deterministic tests separately prove small scans
  do not queue behind a saturated large lane.
