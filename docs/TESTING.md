...
| B22 | Every normally finishing capture process emits one typed `capture.drain` report, including zero-activity and spawn-failure paths. Fixed process/stdio/network/OTLP states distinguish off-by-policy, configured-unavailable, attached-no-data, full, metadata-only, and mixed capture with an explicit evidence trust class. A hard-killed process emits no completion report; unsupported opaque TLS/UDP traffic is metadata-only rather than fabricated as full capture. | Shared-contract unit tests + local JSONL/gRPC mock-harness E2E + transparent relay tests |
