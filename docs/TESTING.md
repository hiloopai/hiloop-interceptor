...
| B23 | An immutable set of exact public HTTPS FQDN:443 destinations may be routed through one fixed, server-authenticated TLS relay. Each matched request reads a fresh sidecar proof, replaces workload authorization and private hiloop headers, preserves the destination authority and streaming body, and emits only safe metadata. Malformed authorities, alternate ports, WebSockets, missing proofs, relay failures, and TLS identity failures fail closed without direct fallback; nonmatches retain ordinary proxy and capture behavior. | Selector/connector unit tests + handler tests + real CONNECT/MITM/TLS/HTTP2 streaming relay test |
