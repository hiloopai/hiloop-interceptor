...
| B20 | A telemetry-gateway outage never aborts capture and never blocks the child: gRPC export failures are classified — transient (`UNAVAILABLE`, `RESOURCE_EXHAUSTED`, transport) park the batch in a bounded in-memory spool (event + byte caps; over-cap drops oldest, counted) redelivered strictly in arrival order under bounded exponential backoff; permanent rejections (`INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAUTHENTICATED`) drop that batch immediately with a loud warning; anything else gets one inline retry, then spools. Local sinks (JSONL) keep capturing regardless. At run end ordinary data and the terminal record each receive one bounded drain phase (31.5 seconds per phase, 63 seconds total with the default policy); loss and undelivered backlog are reported with observed/spooled/landed/dropped/rejected/pending counts on stderr and on the controlled-terminal `capture.drain` attempt. | Spool unit tests + gRPC classification tests + gateway-outage mock-harness E2E |
