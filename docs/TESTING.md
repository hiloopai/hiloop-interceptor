...
| B23 | An immutable set of exact public HTTPS FQDN:443 destinations may be routed through one fixed, server-authenticated TLS relay. Each matched request reads a fresh sidecar proof, replaces workload authorization and private hiloop headers (including request trailers), preserves the exact accepted Host value, destination authority, and streaming body, and emits only safe metadata. Malformed authorities, alternate ports, WebSockets, missing proofs, fragmented or unknown CONNECT protocols, missing or mismatched SNI, relay failures, TLS identity failures, and fixed connect/TLS/response-header deadlines fail closed without direct fallback; nonmatches retain ordinary proxy and capture behavior. | Selector/connector unit tests + handler/trailer tests + real CONNECT/MITM/TLS/HTTP2 streaming relay, fragmented ClientHello, SNI, Host-preservation, and blackhole tests |
