...
| B16 | With `--proxy`, the wrapper injects `HTTPS_PROXY` + a child-scoped CA bundle, decrypts the child's HTTPS, and emits run-lineage-stamped `net`/`llm` events. Ordinary bodies stream frame-by-frame into a content-addressed blob store (`--blob-dir`) and events carry a `payload_ref`; exact supported OAuth token exchanges forward unchanged but emit metadata-only request/response records with query-free targets and no inline body or blob. | Mock-harness MITM E2E + blob/handler/normalizer tests |
...
| B20 | A telemetry-gateway outage never aborts capture and never blocks the child: gRPC export failures are classified — transient (`UNAVAILABLE`, `RESOURCE_EXHAUSTED`, transport) park the batch in a bounded in-memory spool (event + byte caps; over-cap drops oldest, counted) redelivered strictly in arrival order under bounded exponential backoff; permanent rejections (`INVALID_ARGUMENT`, `PERMISSION_DENIED`, `UNAUTHENTICATED`) drop that batch immediately with a loud warning; anything else gets one inline retry, then spools. Local sinks (JSONL) keep capturing regardless. At run end ordinary data receives one bounded drain phase (31.5 seconds with the default policy); only after that prefix settles does the terminal record receive the same bounded phase (63 seconds total when both run). Loss and undelivered backlog are reported with observed/spooled/landed/dropped/rejected/pending counts on stderr and on the controlled-terminal `capture.drain` attempt. | Spool unit tests + gRPC classification tests + gateway-outage mock-harness E2E |
...
| B22 | Every controlled terminal capture path attempts one typed `capture.drain` report, including zero-activity and spawn-failure paths; durable presence certifies a terminal producer receipt, and only `capture.complete=true` certifies complete settled capture. Absence means no durable producer receipt. Transparent capture relays the workload report as typed state and emits exactly one host-composed record with process/stdio/network/OTLP and gateway delivery truth. Fixed source states distinguish off-by-policy, configured-unavailable, attached-no-data, full, metadata-only, and mixed capture with an explicit evidence trust class. A hard-killed process emits no completion report; unsupported opaque TLS/UDP traffic is metadata-only rather than fabricated as full capture. | Shared-contract unit tests + local JSONL/gRPC mock-harness E2E + transparent relay tests |
| B23 | An immutable set of exact public HTTPS FQDN:443 destinations may be routed through one fixed, server-authenticated TLS relay. Each matched request reads a fresh, regular-file, I/O-bounded sidecar proof, replaces workload authorization and private hiloop headers (including request trailers), preserves the exact accepted Host value, destination authority, and streaming body, and emits only safe metadata. Malformed or conflicting authorities, alternate ports, WebSockets, missing proofs, fragmented or unknown CONNECT protocols, missing or mismatched SNI, relay failures, TLS identity failures, and fixed connect/TLS, nonempty-byte request-body idle, plus post-request response-header deadlines fail closed without direct fallback; early response headers do not disable upload supervision, and nonmatches retain ordinary proxy and capture behavior. | Selector/connector unit tests + handler/trailer tests + real CONNECT/MITM/TLS HTTP/1 absolute-form and HTTP/2 streaming relay, fragmented and unknown-preface selected-origin observers, pre-certificate SNI rejection with a trusted matching control, Host-preservation, slow-upload/EOF, over-window and post-early-response stalled uploads, zero-length DATA, regular-file/FIFO, and blackhole tests |
...
policy-matrix tests, direct post-teardown worker-failure persistence under blocked export, cleanup
order, and MTU/fragment policy. A
